...
# Output folders
csv_out_closed <- here("data", "processed")
fig_out_closed <- here("results", "figures", "closed_search")

dir.create(csv_out_closed, recursive = TRUE, showWarnings = FALSE)
dir.create(fig_out_closed, recursive = TRUE, showWarnings = FALSE)

# Load files
closed_raw <- read_tsv(here("data_raw", "ptm_closed_search_noimputation.tsv"))

protein_closed_raw <- read_tsv(here("data_raw", "ptm_closed_search_noimputation_PG_candidates.tsv"))
...
ggsave(file.path(fig_out_closed, "fig3a_unique_sites_pois.png"),
...
ggsave(file.path(fig_out_closed, "fig3b_biological_ptms_tile.png"),
       fig3b, width = 7, height = 5, dpi = 300)


# Load protein candidates

protein_closed <- read_tsv(here("data_raw", "ptm_closed_search_noimputation_PG_candidates.tsv"))
...
ggsave(file.path(fig_out_closed, "fig4a_dynamics_heatmap.png"),
...
# Save
png(file.path(fig_out_closed, "fig4b_site_annotation_table.png"),
